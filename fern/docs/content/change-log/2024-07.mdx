...
You can now see token counts and other usage statistics appear in Prompt Node results when invoking Workflows in the Workflow Sandbox:

![Prompt Node Usage](https://storage.cloud.google.com/vellum-public/help-docs/changelogs/2024-07/prompt-node-usage.png)

This setting is now on by default, but could be toggled off in the Workflow Builder Settings. This setting is also now available for _Workflow Deployments_, by passing in `True`
...
## Enable/Disable All Mocks toggle
