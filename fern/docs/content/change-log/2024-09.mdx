...
When Map Nodes fail, we now retain the pagination state which allows you to view previous iterations of the Map Node.
...
Each of these iterations, included the rejected one, is now also viewable when you expand the Map Node to the full screen editor.
...
The full screen editor now also features the pagination state on the top right to cycle through different iterations of the Map Node while it's already expanded.
