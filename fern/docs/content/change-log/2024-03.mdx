...
Workflow development could become quite expensive with larger workflows due to iteratively rerunning expensive prompts during the early nodes of the Workflow. Often as a user, you already know what kind of output to expect as you develop the rest of the workflow. Available today, you can now _mock_ executions of Prompt and Subworkflow node executions with outputs you already expect. This can help you dramatically speed up your Workflow development during the later nodes of your Workflow since you no longer have to wait for early Prompt nodes to complete and could save those tokens.
