...
We had previously supported "error outputs" for all output generating Workflow Nodes, which enabled you to add error handling natively to your Workflows. However, this made debugging downstream nodes more difficult if you weren't already handling this new error output. We now include a new `Reject on Error` toggle on each of these data generating nodes to solve this issue:

![Workflow Node Reject on Error](https://storage.googleapis.com/vellum-public/help-docs/node-error-toggle.png)

When enabled, you are opting out of handling the error in the workflow and the workflow will reject immediately with the relevant error. This will be the default behavior for new Workflow Nodes going forward.
