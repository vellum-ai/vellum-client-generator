...
Image support is LIVE in the Vellum UI for OpenAI's GPT-4 Turbo with Vision! To use this feature, drag a publicly hosted image url (that includes one of OpenAI's acceptable image formats) into a Prompt or Workflow scenario's Chat History message in the Sandbox.
This converts your chat message from a string to an array and creates a thumbnail of your image in the message. Click the gear icon to open the image configuration where you can update the image detail or replace the image url.

For more details on supported image formats and working with OpenAI's vision models in Vellum, check out our [new help center doc](https://docs.vellum.ai/help-center/prompts/working-with-images-in-vellum).

![Image Support in Vellum UI](https://storage.googleapis.com/vellum-public/help-docs/workflow-node-inputs.gif)
