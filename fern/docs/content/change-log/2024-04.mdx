...
We have added the ability for you to track model host usage from the `execute-prompt` [API](/api-reference/api-reference/test-suites/list-test-suite-test-cases). This API update is available on version `0.3.21` of our SDKs.
...
We have a new [API](/api-reference/api-reference/test-suite-runs/retrieve) available in beta for listing the Test Cases belonging to a Test Suite at `GET /v1/test-suites/{id}/test-cases`.
