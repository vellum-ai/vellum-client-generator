...
We have added the ability for you to track model host usage from the `execute-prompt` [API](/api-reference/api-reference/test-suite-runs/retrieve) .

This API update is available on version `0.3.21` of our SDKs.

You can also now view model host usage in the Prompt Sandbox by enabling the "Track Usage" toggle in Prompt Sandbox's settings.
