...
Previously, editing past versions of a Prompt Sandbox was confusing, with unclear indications of which version you were modifying and how it was being saved. Now, the history view for a Prompt Sandbox is read-only. To edit a previous version, simply click 'Restore,' and a new version will be created from that point.
