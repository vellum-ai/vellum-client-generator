...
Earlier this month, we restricted the Workflow Deployment Executions table to only show executions invoked via an API Request. This allowed users to filter out all of the noise
of all the other contexts in which a Workflow Deployment can be invoked and focus on only production data. However, there are still other contexts in which it's useful to still
see Workflow Executions.

You can now see a new `Executed By` column that shows what was the immediate "parent" context by which a Workflow was executed. By default, this table will be filtered on `API Request`.
However we also support all the other contexts for users interested in seeing more contexts in which a Workflow was executed, such as via another Workflow Deployment:
