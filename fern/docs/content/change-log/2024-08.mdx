...
Previously, our Vellum Prompts could accept dynamic strings and chat histories as inputs to their template.
...
Vellum Prompts now support native JSON as inputs! When you add a new input variable to a Prompt, you can now select the "JSON" type.
...
JSON input values will render as prettified JSON objects when used in Rich Text Blocks and can be operated on directly
without the need for `json.loads()` within Jinja Blocks.
