...
If you wanted to drill into a single prompt, previously your only recourse was to navigate to the Prompt Deployment's executions table and try to filter for the id
you're looking for. Now each row has a navigable link accessible on the table: 

![Prompt Execution Link](https://storage.googleapis.com/vellum-public/help-docs/changelogs/2024-08/prompt-execution-link.png)

This will navigate you to a dedicated page representing thie Prompt's execution. From here, you could inspect the raw HTTP data sent to and from the provider,
