...
Our Workflow Deployment Executions page used to list all executions of a Workflow Deployment, no matter where it was invoked from. However, this
would often get confusing because it would mix results from evaluations and results from production into the same view. Our Workflow Deployment Executions
page now filters down to just the executions invoked from the API. Executions from evaluations are still accessible by hovering over a row and clicking
the "View Workflow Details" button:
