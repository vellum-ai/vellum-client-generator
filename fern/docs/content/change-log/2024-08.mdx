...
Anthropic recently released API changes to allow for [Prompt Caching](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching#how-prompt-caching-works).
This new feature allows for caching of content for up to 5 minutes; optimizing for processing time and costs for repetitive tasks
or consistent elements.

This powerful feature is now integrated with Vellum! In order to use, simply toggle the new cache options on the prompt blocks for the supported
Claude Sonnet 3.5 and Claude Haiku 3.0 .
