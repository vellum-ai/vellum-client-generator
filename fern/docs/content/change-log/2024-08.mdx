...
Previously our JSON Schema editor only supported a form UI and file upload capabilities. You now have access to edit your JSON Schema _directly_ using the raw editor below:

![Raw Schema Button](https://storage.googleapis.com/vellum-public/help-docs/changelogs/2024-08/raw_schema_button.png)

From here, you can edit the JSON schema directly, giving you access to all features supported by the [JSON Schema spec](https://json-schema.org/overview/what-is-jsonschema) that may not yet be supported by our Form component, such
as references:
