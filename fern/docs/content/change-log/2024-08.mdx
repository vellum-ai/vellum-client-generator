...
We now support using Anthropic's Claude 3.5 Sonnet, Claude 3 Opus and Claude 3 Haiku Models using [Google Vertex AI](https://cloud.google.com/vertex-ai). You can add them to your workspace from the [models page](https://app.vellum.ai/models).
...
We now support using Anthropic's Tool Use API for function calling. Previously Anthropic function calling had been supported by shimming function call XML into the prompt.
