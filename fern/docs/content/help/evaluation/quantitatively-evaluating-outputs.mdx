...
3. From the select metric type dropdown, select JSON Schema Match. To learn about metric types other than JSON Schema Match, see Vellum's [Available Metric Types](#available-metric-types).
4. In the "Label" field at the top left, enter "My First Metric". The name should autopopulate. This is a unique name that you can use to programmatically identify this metric.
