...
Workflow development is best done iteratively. However, this could become prohibitively expensive both due to token consumption and runtime if there are Prompt Nodes defined early in the Workflow. To help speed up Workflow development, you could define _mock_ executions for a given node. This will use the output you define for this node instead of running the node itself.

![Workflow Node Mocking](https://storage.googleapis.com/vellum-public/help-docs/workflow-node-mocking.png)

Once defined, you could easily toggle the mock on and off to go back and forth between mocking the node and running the real Prompt to see your Workflow work end-to-end. This also allows you to save your mocks without needing to delete them when running the real node. During a workflow run, nodes that are marked will be outlined it's own color to differentiate itself from nodes that are actually executed.

![Workflow Node Mocking](https://storage.googleapis.com/vellum-public/help-docs/node-mocking-result.png)

These mocks are _only_ defined within the context of Workflow Sandboxes, and are defined per scenario. They do not get deployed with your Workflow Deployments.
